...
\newcommand{\stack}[4]{
  \foreach \i in {1,...,#1} {
    \draw[fill=white] #2 ++({0.1*(#1)},{-0.1*(#1)}) ++({-0.1*\i},{0.1*\i}) rectangle +#3;
  }
...
\stack{4}{(-1,-1)}{(1,1)}{} \node (input) at (-0.5,-0.5) {Input}; 

% Input conv layer
\foreach \m/\l [count=\y] in {1,2,missing,3}
  \node [every filter/.try, filter \m/.try, minimum size=1cm] (conv1-\m) at (2,2.25-\y*1.25) {};

% Second conv layer
\foreach \m [count=\y] in {1,2,3,missing,4}
  \node [every filter/.try, filter \m/.try ] (conv2-\m) at (4,2.5-\y*1.1) {};

% Hidden layer
\foreach \m [count=\y] in {1,2,3,4,5,missing,6}
  \node [every neuron/.try, neuron \m/.try, minimum size=0.25cm] (hidden-\m) at (6,2-\y*0.7) {};

% Output layer
\foreach \m [count=\y] in {1,2,3,4,5,6}
  \node [every neuron/.try, neuron\m/.try, minimum size=0.4cm] (output-\m) at (8, 1.7-\y*0.7){};
...
\foreach \I [count=\i] in {1,...,6}
 \draw [->] (output-\i) -- ++(1,0)
  node [above, midway, font=\footnotesize] {$Q_{a = \I}$};


\foreach \l [count=\x from 0] in {Convolution, Convolution, Hidden, Output}
  \node [align=center, above] at (2 + \x*2,2) {\l \\ layer};

\foreach \l [count=\x from 0] in {16 $8\times8$\\ filters, 32 $4\times4$\\ filters, 256\\ units, One output\\ per action}
  \node [align=center, below, font=\footnotesize] at (2 + \x*2,-3.5) {\l};
